...
const Register = ({ params }: any) => {
  ...
  useEffect(() => {
    ...
    const formPwConfirm = document.getElementById(
      "pw-confirm"
    ) as HTMLInputElement;
    const formTerms = document.getElementById(
      "terms-check"
    ) as HTMLInputElement;
    const formPrivacy = document.getElementById(
      "privacy-check"
    ) as HTMLInputElement;
    ...
    if (formPwConfirm.value !== "") {
      setPwConfirmVal(formPwConfirm.value);
    }
  ...
  const handleValidateChk = async () => {
    ...
    await fetch(url + query, options)
      ...
      .then((data) => {
        ...
        if (idVal === "") {
          setIdChk("empty");
        } else if (!emailPattern.test(idVal)) {
          setIdChk("wrong-pattern");
        } else if (isIdExists) {
          setIdChk("is-used");
        }
        ...
        if (pwVal === "") {
          ...
          setPwChk("wrong-pattern");
        }

        // 비밀번호 재입력 유효성 검사
        if (pwConfirmVal === "") {
          setPwConfirmChk("empty");
        } else if (pwConfirmVal !== pwVal) {
          setPwConfirmChk("unmatch");
        }
      })
    ...
    if (
      nameVal !== "" &&
      idVal !== "" &&
      emailPattern.test(idVal) &&
      !isIdExists &&
      pwVal !== "" &&
      pwPattern.test(pwVal) &&
      pwConfirmVal !== "" &&
      pwConfirmVal === pwVal
    ) {
      return true;
              ...
              <div className="w-full flex flex-col">
                <div className="w-full flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <label
                      htmlFor="terms-check"
                      className="group w-5 h-5 text-lg flex justify-center items-center cursor-pointer"
                    >
