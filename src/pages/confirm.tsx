      ...
      <div className="w-full h-full absolute left-0 top-0 flex justify-center items-center text-center text-l-2 dark:text-white">
        <div className="w-[22.5rem]">
          ...
          <div
            ...
            className="w-full px-7 py-4 mt-5 text-left leading-loose rounded-lg text-sm bg-h-1/20 dark:bg-f-8/20 border border-dashed border-h-1 dark:border-f-8"
          >
          ...
          <div className="flex justify-center items-center mt-10">
            <Link
              ...
              className="flex justify-center items-center w-[8.25rem] h-9 rounded-lg text-sm border border-h-1 dark:border-f-8 hover:bg-h-1 hover:dark:bg-f-8 text-h-1 dark:text-f-8 hover:text-white hover:dark:text-d-2 cursor-pointer duration-100"
            >
              메인 페이지
            </Link>
          </div>
...
export async function getServerSideProps(ctx: any) {
  try {
    ...
    // token으로 유저 정보 불러오기
    const user =
      token === undefined ? null : await FetchUserInfoFromToken(token);
