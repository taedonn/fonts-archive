        ...
        if (req.query.action === "login") {
            try {
                ...
                status === "success" && user!== null && (
                    ...
                    res.setHeader(
                        'Set-Cookie',
                        `refreshToken=${refreshToken}; Path=/; max-Age=31536000; Expires=${stayLoggedIn ? date.setFullYear(date.getFullYear() + 1) : date.setDate(date.getDate() + 1)}; HttpOnly; secure=true; SameSite=Strict;`,
                    )
                );
