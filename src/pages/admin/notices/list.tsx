...
// api
import { FetchNotices, FetchNoticesLength } from "@/pages/api/admin/notices";
...
const NoticeList = ({ params }: any) => {
  ...
  const handlePageChange = (e: React.ChangeEvent<unknown>, value: number) => {
    router.push(
      `/admin/notices/list${value === 1 ? "" : `?page=${value}`}${
  ...
  const handleSearchClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    router.push(
      `/admin/notices/list${filter === "all" ? "" : `?filter=${filter}`}${
        input.value === ""
          ? ""
