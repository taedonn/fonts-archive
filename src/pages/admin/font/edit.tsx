...
const Edit = ({ params }: any) => {
  ...
  const [showType, setShowType] = useState(
    defaultFont !== null ? defaultFont.show_type : false
  );
  ...
  const { isLoading, isRefetching, isSuccess, refetch } = useQuery(
    ["font-search"],
    async () => {
  ...
  useEffect(() => {
    function handleAccountOutside(e: Event) {
      if (
        refSearchBar?.current &&
        !refSearchBar.current.contains(e.target as Node) &&
        refSearchResult.current &&
        !refSearchResult.current.contains(e.target as Node)
      ) {
        setFocus(false);
      }
    }
  ...
  useEffect(() => {
    setShowType(defaultFont !== null ? defaultFont.show_type : false);
  }, [defaultFont]);
          ...
          <div className="max-w-[45rem] w-full flex flex-col justify-center items-start my-16 lg:my-24 mt-8 lg:mt-16">
            ...
            <div className="w-full p-5 rounded-lg bg-l-e dark:bg-d-3 drop-shadow-default dark:drop-shadow-dark">
              <div className="w-full relative">
                <label
                  htmlFor="search-font"
                  className="block ml-px font-medium"
                >
                  폰트 검색
                </label>
                <input
                ...
                <div
                  ...
                  className="custom-sm-scrollbar w-full h-44 py-2.5 overflow-y-auto absolute z-10 -bottom-1.5 translate-y-full rounded-lg text-sm drop-shadow-default dark:drop-shadow-dark bg-l-d dark:bg-d-4"
                >
              ...
              <div className="flex flex-col">
                ...
                <div className="w-max h-12 rounded-lg mt-2 px-3.5 flex items-center text-sm bg-l-d dark:bg-d-4">
                  ...
                  <div
                    className={`${
                      showType ? "text-h-1 dark:text-f-8" : ""
                    } ml-1.5`}
                  >
                    보임
                  </div>
                ...
                >
                  폰트 언어
                  <div className="ml-2 font-normal text-sm text-h-1 dark:text-f-8">
                    [KR, EN]
                  </div>
                </label>
                <TextInput
    ...
    src: url('https://cdn.jsdelivr.net/gh/fonts-archive/NanumSquare/NanumSquare.woff2') format('woff2'),
            ...
            url('https://cdn.jsdelivr.net/gh/fonts-archive/NanumSquare/NanumSquare.ttf') format('truetype');
                ...
                <TextInput
                  ...
                  value={
                    defaultFont !== null
                      ? defaultFont.license_print +
                        defaultFont.license_web +
                        defaultFont.license_video +
                        defaultFont.license_package +
                        defaultFont.license_embed +
                        defaultFont.license_bici +
                        defaultFont.license_ofl +
                        defaultFont.license_purpose +
                        defaultFont.license_source
                      : ""
                  }
                  id="font-license"
                  tabindex={17}
...
export async function getServerSideProps(ctx: any) {
  try {
    ...
    if (
      ...
      // 쿼리가 있으면 폰트 불러오기
      const font = ctx.query.code
        ? await FetchFont(Number(ctx.query.code))
        : null;
