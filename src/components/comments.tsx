...
// global
import { comments, reports } from "@/libs/global";
...
export default function Comments({
  ...
  const handleHeightChangeOnClick = () => {
    if (commentRef.current) {
      ...
      commentRef.current.style.height =
        commentRef.current.scrollHeight + 4 + "px";
    }
  };
  ...
  const newComment = async (e: React.MouseEvent<HTMLButtonElement>) => {
    if (
      commentRef.current &&
      e.currentTarget.classList.contains("comment-enabled")
    ) {
  ...
  const commentEditShow = (e: any) => {
    ...
    const commentWrap = document.getElementById(
      "comment-btn-wrap-" + id
    ) as HTMLDivElement;
    ...
    if (e.target.checked) {
      ...
      editor.getElementsByTagName("textarea")[0].value =
        comment.getElementsByTagName("pre")[0].innerHTML;
  ...
  const compareIdsFromReports = (reports: reports[]) => {
    const isReported =
      reports.length === 0
        ? false
        : reports.some(
            (report: reports) => report.reported_user_id === Number(user.id)
          );
    return isReported;
  };
  ...
  useEffect(() => {
    function handleOutside(e: Event) {
      ...
      if (
        shareExpandBtn.current &&
        !shareExpandBtn.current.contains(e.target as Node) &&
        shareExpandContent.current &&
        !shareExpandContent.current.contains(e.target as Node)
      ) {
        setShareExpand(false);
        input.checked = false;
      }
  ...
  const copyUrl = () => {
    ...
    const copyBtn = document.getElementsByClassName(
      "url_copy_btn"
    )[0] as HTMLDivElement;
    const copyChkBtn = document.getElementsByClassName(
      "url_copy_chk_btn"
    )[0] as SVGSVGElement;
  ...
  // 공유할 url
  const url =
    "https://fonts.taedonn.com/post/" + font.font_family.replaceAll(" ", "+");
  const urlEncoded =
    "https://fonts.taedonn.com/post/" + font.font_family.replaceAll(" ", "%2B");
  ...
  const shareKakao = async () => {
    ...
    await Kakao.Share.sendDefault({
      ...
      content: {
        ...
        imageUrl: `https://fonts-archive-meta-image.s3.ap-northeast-2.amazonaws.com/${font.font_family.replaceAll(
          " ",
          ""
        )}.png`,
        link: {
  ...
  const shareLine = () => {
    window.open(
      `https://social-plugins.line.me/lineit/share?url=${decodeURI(
        urlEncoded
      )}`,
      "line",
      "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=520,height=700"
    );
  };
  ...
  const shareTwitter = () => {
    ...
    window.open(
      `https://twitter.com/intent/tweet?text=${text}&url=${decodeURI(
        urlEncoded
      )}`,
      "x",
      "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=520,height=700"
  ...
  return (
    <>
      ...
      <div className="w-max mb-3 flex gap-2 text-l-2 dark:text-white">
        ...
        >
          {!likedInput ? (
            <div className="w-14 h-8 flex justify-center items-center rounded-lg bg-l-f hover:bg-l-e dark:bg-d-3 hover:dark:bg-d-4">
              <i className="text-sm bi bi-heart"></i>
              <div className="ml-1.5 text-[0.813rem] selection:bg-transparent">
                {likedNum}
              </div>
            </div>
          ) : (
        ...
        <div className="relative">
          ...
          <label
            ref={shareExpandBtn}
            htmlFor="share-expand"
            onMouseDown={(e) => onMouseDown(e, 0.9, true)}
            onMouseUp={onMouseUp}
            onMouseOut={onMouseOut}
            className="w-max h-8 px-3 text-[0.813rem] flex justify-center items-center cursor-pointer rounded-lg selection:bg-transparent bg-l-f hover:bg-l-e peer-checked:bg-l-e dark:bg-d-3 hover:dark:bg-d-4 peer-checked:dark:bg-d-4"
          >
          ...
          {shareExpand && (
            <div
              ref={shareExpandContent}
              className="p-4 rounded-lg absolute left-0 -top-2 -translate-y-full bg-l-f dark:bg-d-3"
            >
              <div className="w-full mb-4 relative rounded-lg flex items-center overflow-hidden">
                <input
                  type="text"
                  id="url"
                  defaultValue={`https://fonts.taedonn.com/post/${font.font_family.replaceAll(
                    " ",
                    "+"
                  )}`}
                  className="w-full h-12 px-3 pr-10 py-2 rounded-r-lg text-sm placeholder-l-5 dark:placeholder-d-c bg-l-d/60 dark:bg-d-6/60"
                />
                <label
                  onClick={copyUrl}
                  htmlFor="url"
                  className="w-8 h-8 absolute z-10 right-1 flex justify-center items-center cursor-pointer hover:text-h-1 hover:dark:text-f-8"
                >
              ...
              <div className="gap-3 flex justify-center items-center text-xs text-l-5 dark:text-d-c">
                ...
                >
                  ...
                  <div className="w-12 mt-2.5 text-center group-hover:text-l-2 group-hover:dark:text-white">
                    카카오톡
                  </div>
                </button>
                <button
      ...
      <h2 className="font-medium mb-4">
        댓글 {comments === null ? 0 : comments.length}개
      </h2>
          ...
          {user === null ? (
            ...
            <div className="w-full flex gap-4">
              ...
              <div className="w-full flex flex-col mt-0.5 lg:mt-1.5">
                <div
                  className={`relative w-full flex items-center pb-1.5 border-b hover:border-l-9 hover:dark:border-d-9 ${
                    commentFocus
                      ? "border-l-9 dark:border-d-9"
                      : "border-l-b dark:border-d-6"
                  }`}
                >
                  <textarea
                    ref={commentRef}
                    ...
                    placeholder="댓글 달기..."
                    className="w-full h-7 resize-none placeholder-l-5 dark:placeholder-d-c bg-transparent"
                  />
                </div>
                {commentFocus && (
                  <div className="flex gap-2 w-full mt-3 text-sm">
                    <button
                      ref={commentBtnRef}
                      onMouseDown={newComment}
                      className={`${
                        commentBtn
                          ? "comment-enabled text-white dark:text-f-1 bg-h-1 hover:bg-h-0 dark:bg-f-8 hover:dark:bg-f-9 cursor-pointer"
                          : "comment-disabled text-l-9 dark:text-d-9 bg-l-f dark:bg-d-3 cursor-default"
                      } w-14 h-8 rounded-lg`}
                    >
                      댓글
                    </button>
                    <button
                      onMouseDown={commentCancelBtnOnMouseDown}
                      className="w-14 h-8 rounded-lg hover:bg-l-f hover:dark:bg-d-3"
                    >
                      취소
                    </button>
                  </div>
      ...
      <div className="w-full min-h-[7.5rem] mb-36 lg:mb-40 lg:pl-10">
        {comments === null || comments.length === 0 ? (
          <div className="w-full mt-4 text-center text-l-5 dark:text-d-c">
            아직 댓글이 없습니다.
          </div>
        ) : (
          <>
            {comments.map((comment: any) => {
              return (
                <div
                  ...
                  className="w-full animate-fade-in-fontbox"
                >
                    ...
                    <div className="w-full">
                      <div className="flex gap-1 lg:gap-3 tlg:flex-col lg:items-center">
                        <div className="font-medium">
                          {comment.user_auth !== "credentials" &&
                          comment.user_privacy
                            ? hideUserName(comment.user_name, 1)
                            : comment.user_name}
                        </div>
                        <div className="flex gap-3 items-center">
                          <div className="text-sm text-l-5 dark:text-d-c">
                          ...
                          {user ? (
                            ...
                            !comment.is_deleted_by_reports ? (
                              !compareIdsFromReports(comment.reports) ? (
                                <button
                                  id={`report-comment-${comment.comment_id}`}
                                  data-user={comment.user_id}
                                  onClick={reportCommentModalOpen}
                                  className="flex gap-1 items-center hover:text-h-1 hover:dark:text-f-8"
                                >
                              ...
                              !comment.is_deleted_by_reports ? (
                              <div
                                id={`comment-btn-wrap-${comment.comment_id}`}
                              ...
                              >
                                ...
                                />
                                <label
                                  htmlFor={`comment-edit-${comment.comment_id}`}
                                  className="flex gap-1 items-center cursor-pointer hover:text-h-1 hover:dark:text-f-8"
                                >
                      ...
                      >
                        {comment.is_deleted_with_reply ? (
                          <div className="text-l-5 dark:text-d-c">
                            [삭제된 댓글입니다]
                          </div>
                        ) : comment.is_deleted_by_reports ? (
                          <div className="text-l-5 dark:text-d-c">
                        ...
                        <input
                          onChange={commentReplyShow}
                          id={`comment-reply-${comment.comment_id}`}
                          type="checkbox"
                          className="hidden peer"
                        />
                        <label
                          htmlFor={`comment-reply-${comment.comment_id}`}
                          className={`${
                            user ? "block" : "hidden"
                          } peer-checked:hidden mt-3 text-h-1 dark:text-f-8 hover:underline cursor-pointer`}
                        >
                          답글
                        </label>
                      </div>
                      ...
                      >
                        ...
                        <div className="flex gap-2 text-sm mt-3">
                          <button
                            onClick={editCommentAPIInit}
                            id={`comment-edit-btn-${comment.comment_id}`}
                            onMouseDown={(e) =>
                              onMouseDown(
                                e,
                                0.9,
                          ...
                          <button
                            onClick={commentEditCancelBtnOnClick}
                            id={`comment-edit-cancel-${comment.comment_id}`}
                            onMouseDown={(e) => onMouseDown(e, 0.9, true)}
                            onMouseUp={onMouseUp}
                            onMouseOut={onMouseOut}
                            className="w-14 h-8 rounded-lg hover:bg-l-f hover:dark:bg-d-3"
                      ...
                      {user ? (
                        <div
                          id={`comment-reply-content-${comment.comment_id}`}
                          ...
                          <div className="w-full flex gap-4">
                            ...
                            <div className="w-full">
                              <div className="relative w-full flex items-center pb-1.5 border-b border-l-b dark:border-d-6">
                                <textarea
                                  onInput={handleHeightChange}
                                  onChange={commentReplyOnChange}
                                  onFocus={commentReplyOnChange}
                                  id={`comment-reply-textarea-${comment.comment_id}`}
                                  placeholder="답글 달기..."
                                  className="w-full h-7 resize-none mt-1.5 placeholder-l-5 dark:placeholder-d-c bg-transparent"
                                />
                              </div>
                              <div className="flex gap-2 text-sm mt-3">
                                <button
                                  onClick={replyCommentAPIInit}
                                  id={`comment-reply-btn-${comment.comment_id}`}
                                  data-comment={comment.comment_id}
                                  data-bundle={comment.bundle_id}
                                  data-email={comment.user_email}
                                  data-auth={comment.user_auth}
                                  onMouseDown={(e) =>
                                    onMouseDown(
                                      e,
                                      0.9,
                                  ...
                                  onMouseUp={onMouseUp}
                                  onMouseOut={onMouseOut}
                                  className="edit-btn-disabled w-14 h-8 rounded-lg bg-l-f dark:bg-d-3 text-l-9 dark:text-d-9 cursor-default"
                                >
                                  답글
                                </button>
                                <button
                                  onClick={commentReplyCancelBtnOnClick}
                                  id={`comment-reply-cancel-${comment.comment_id}`}
                                  onMouseDown={(e) => onMouseDown(e, 0.9, true)}
                                  onMouseUp={onMouseUp}
                                  onMouseOut={onMouseOut}
                                  className="w-14 h-8 rounded-lg hover:bg-l-f hover:dark:bg-d-3"
