            ...
            <header className="w-full h-16 text-l-2 dark:text-white">
                ...
                <div className='w-full h-16 px-4 lg:px-8 fixed right-0 top-0 z-20 flex justify-between items-center bg-white dark:bg-d-2'>
                    ...
                    <div className='relative flex shrink-0 items-center'>
                        ...
                        <button onClick={handleFontSearch} onMouseDown={e => onMouseDown(e, 0.95, true)} onMouseUp={onMouseUp} onMouseOut={onMouseOut} className={`${page === "index" ? "hidden" : "flex"} w-56 tlg:w-8 h-8 lg:pl-10 mr-1.5 lg:mr-3 relative text-sm items-center rounded-lg text-h-1 dark:text-white lg:hover:text-white lg:hover:dark:text-f-8 bg-h-e dark:bg-d-3 lg:hover:bg-h-1 lg:hover:dark:bg-d-4`}>
                        ...
                        <div className="relative lg:mr-1">
                            ...
                            {
                                alertsDisplay
                                && <div ref={refAlertDiv} id="alert-popup" className="animate-fade-in-account w-80 min-h-20 absolute -right-[4.75rem] lg:-right-[5.5rem] top-12 py-3 pb-12 rounded-lg drop-shadow-default dark:drop-shadow-dark cursor-default bg-white dark:bg-d-3">
                                    <div className="mb-3 px-4 w-full flex justify-between text-xs">
                                        ...
                                        { user && <button onClick={handleReadAllAlerts} className="text-h-1 dark:text-f-8 lg:hover:underline">모두 읽음 표시</button> }
                                    </div>
                                    <div className="w-full max-h-80 overflow-y-auto custom-sm-scrollbar border-y border-l-b dark:border-d-6">
                                                                ...
                                                                page.alerts.map((alert: alerts) => (
                                                                    <div key={alert.alert_id} className={`${alert.alert_read ? "" : "bg-h-e dark:bg-f-8/10"} w-full p-4 flex gap-3 border-b last:border-transparent border-l-d dark:border-d-4`}>
                                                                        ...
                                                                        <div className="flex flex-col gap-1.5">
                                                                            ...
                                                                            <Link href={alert.alert_link} className="flex flex-col lg:hover:underline">
                                                                                <div className="ellipsed-text">{alert.sender_content}</div>
                                                                            </Link>
                                                                            <div className="text-xs flex gap-2">
                                                                                <div className="text-l-9">{timeDiff(alert.created_at.toString())}</div>
                                                                                <button onClick={() => handleReadAlerts(alert.alert_id, alert.alert_read)} className={`${alert.alert_read ? "text-l-9 cursor-default" : "text-h-1 dark:text-f-8 lg:hover:underline"}`}>{alert.alert_read ? "읽음" : "읽음으로 표시"}</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                ))
                                        ...
                                        {/* 알림 더 보기 */}
                                        {
                                            ...
                                            && <div className="mt-5 mb-5 w-full flex gap-1.5 justify-center items-center text-sm text-h-1 dark:text-f-8">
                                                <button onClick={fetchNextAlerts} className={`${isLoading && "cursor-default"} lg:hover:underline mt-0.5`}>알림 더 보기</button>
                        ...
                        <div className="relative flex justify-center items-center cursor-pointer">
                            ...
                            <div ref={refAccountDiv} id="account-select" className="hidden peer-checked:block w-32 absolute right-0 top-10 px-4 py-3 rounded-lg drop-shadow-default dark:drop-shadow-dark cursor-default bg-l-f dark:bg-d-3">
                                <Link href="https://github.com/fonts-archive" target="_blank" rel="noopener noreferrer" className="flex items-center text-sm lg:hover:text-l-5 lg:hover:dark:text-d-c selection:bg-transparent">
                                    <i className="fa-brands fa-github"></i>
                                    <span className="ml-1.5">깃허브</span>
                                </Link>
                                <Link href="/notices" className="flex items-center mt-1.5 text-sm lg:hover:text-l-5 lg:hover:dark:text-d-c selection:bg-transparent">
                                    <i className="fa-solid fa-bell-concierge"></i>
                                    <span className="ml-1.5">공지사항</span>
                                </Link>
                                <Link href="/issue" className="flex items-center mt-1.5 text-sm lg:hover:text-l-5 lg:hover:dark:text-d-c selection:bg-transparent">
                                ...
                                </Link>
                                {
                                    ...
                                    : <div className="text-sm">
                                        ...
                                        <Link href="/user/info" className="flex items-center mt-1.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                            <i className="text-base mr-1 bi bi-person-fill-gear"></i>
                                            프로필 정보
                                        </Link>
                                        <Link onClick={reset} href="/?filter=liked" className="flex items-center mt-0.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                            <i className="ml-px mr-1.5 fa-regular fa-heart"></i>
                                            좋아요 목록
                                        </Link>
                                        <Link href="/user/comments" className="flex items-center mt-1 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                        ...
                                        </Link>
                                        {
                                            ...
                                            ? <>
                                                <div className="w-full h-px bg-l-b dark:bg-d-6 my-2.5"></div>
                                                <div>관리자<span className="text-l-5 dark:text-d-c"> 기능</span></div>
                                                <Link href="/admin/font/list" className="flex items-center mt-1.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-px mr-2 fa-solid fa-a"></i>
                                                    폰트 목록
                                                </Link>
                                                <Link href="/admin/font/add" className="flex items-center mt-0.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-px mr-2 fa-solid fa-a"></i>
                                                    폰트 추가
                                                </Link>
                                                <Link href="/admin/font/edit" className="flex items-center mt-0.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-px mr-2 fa-solid fa-a"></i>
                                                    폰트 수정
                                                </Link>
                                                <Link href="/admin/user/list" className="flex items-center mt-0.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="mr-1 text-base bi bi-person-fill-gear"></i>
                                                    유저 목록
                                                </Link>
                                                <Link href="/admin/comment/list" className="flex items-center mt-0.5 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-px mr-1.5 fa-regular fa-comment"></i>
                                                    댓글 목록
                                                </Link>
                                                <Link href="/admin/issue/list" className="flex items-center mt-1 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-px mr-2 text-xs fa-regular fa-paper-plane"></i>
                                                    문의 목록
                                                </Link>
                                                <Link href="/admin/notices/add" className="flex items-center mt-1 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-0.5 mr-2 text-xs fa-regular fa-flag"></i>
                                                    공지 추가
                                                </Link>
                                                <Link href="/admin/notices/list" className="flex items-center mt-1 lg:hover:text-l-5 lg:hover:dark:text-d-c">
                                                    <i className="ml-0.5 mr-2 text-xs fa-regular fa-flag"></i>
                                                    공지 목록
                                                </Link>
                                            </> : <></>
