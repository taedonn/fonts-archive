...
export default function SelectBox({
  ...
  useEffect(() => {
    function handleClickOutside(e: Event) {
      ...
      if (
        ...
        refOption.current &&
        !refOption.current.contains(e.target as Node)
      ) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.checked) {
      if (window.innerHeight <= labelBottom + optionHeight) {
        ...
        setTimeout(function () {
          option.classList.remove("animate-fade-in-account");
        }, 600);
      }
    }
  };
  ...
  const fetchOptionName = (select: string) => {
    const fetchedOption = options.find(
      (option: any) => option.value === select
    );
  ...
  return (
    <div className="custom-sm-scrollbar w-full relative">
      ...
      <label
        ...
        className="group px-4 flex justify-between items-center rounded-lg border-2 cursor-pointer text-l-2 dark:text-white hover:bg-l-e hover:dark:bg-d-4 peer-checked:hover:bg-transparent border-transparent peer-checked:border-h-1 peer-checked:dark:border-f-8"
      >
        <div className="flex items-center gap-3">
          {icon !== "none" && <i className={`text-lg bi ${icon}`}></i>}
          <span className="font-medium selection:bg-transparent">
            {title}
            <span className="ml-1 font-normal text-h-1 dark:text-f-8">
              [{fetchOptionName(select)}]
            </span>
          </span>
        </div>
      ...
      <div
        ...
        style={{
          height:
            options.length >= 5 ? "13.5rem" : options.length * 2.5 + 1 + "rem",
        }}
