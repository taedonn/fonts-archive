...
export default function ReportCommentModal({
  ...
  /** 사유 state */
  const reportTextChk = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const reportComment = async () => {
    ...
    if (!reportNickname && !reportPolitics && !reportSwearing && !reportEtc) {
    ...
    <>
      {display && (
        <div className="w-full h-full fixed left-0 top-0 z-40 pt-24 tlg:pt-16 tlg:px-4 flex flex-col justify-start items-center backdrop-blur">
          <div
            ref={thisModal}
            className="w-96 txs:w-full p-6 relative rounded-lg animate-zoom-in drop-shadow-default dark:drop-shadow-dark bg-l-e dark:bg-d-3"
          >
            <div className="w-full flex flex-col">
              <div className="font-medium text-l-2 dark:text-white">
                신고하기
              </div>
              <button
                onClick={close}
              ...
              <div className="w-full mt-5 p-5 rounded-lg text-l-2 dark:text-white bg-l-d dark:bg-d-4">
                ...
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    <div className="font-medium ml-2 select-none">
                      부적절한 닉네임
                    </div>
                  </label>
                  <div className="text-sm ml-6 mt-1 text-l-5 dark:text-d-c">
                    선정적이거나, 폭력적인 뜻이 내포된 닉네임
                  </div>
                  <label
                    htmlFor="report-propaganda"
                    className="mt-4 flex items-center cursor-pointer"
                  >
                    <input
                      onChange={reportPoliticsChk}
                      type="checkbox"
                      id="report-propaganda"
                      className="peer hidden"
                    />
                    <i className="block peer-checked:hidden text-lg leading-none text-h-1 dark:text-f-8 fa-regular fa-square-check"></i>
                    <i className="hidden peer-checked:block text-lg leading-none text-h-1 dark:text-f-8 fa-solid fa-square-check"></i>
                  ...
                  <div className="text-sm ml-6 mt-1 text-l-5 dark:text-d-c">
                    자세한 사유는 상세 입력칸에 적어주세요.
                  </div>
                  <textarea
                    onChange={reportTextChk}
                    placeholder="사유는 최대한 자세하게 기입해주세요..."
                    className="custom-sm-scrollbar w-full h-24 resize-none mt-4 px-3 py-2 text-sm border-2 rounded-lg bg-l-e dark:bg-d-3 border-l-b dark:border-d-6 focus:border-h-1 focus:dark:border-f-8 text-l-2 dark:text-white placeholder-l-5 dark:placeholder-d-c"
                  ></textarea>
                </div>
                {reportWarning && (
                  <div className="flex">
              ...
              <div className="flex justify-between gap-2 mt-4">
                <div className="w-1/2">
                  <Button color="gray">
                    <button onClick={reportClose} className="w-full h-full">
                      취소
                    </button>
                  </Button>
                </div>
