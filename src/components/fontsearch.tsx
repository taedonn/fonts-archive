        ...
        <>
            {
                ...
                ? <div className="w-[100%] h-[100vh] fixed left-0 top-0 z-40 flex flex-col justify-start items-center pt-[12vh] tlg:pt-[10vh] tmd:pt-[60px] backdrop-blur bg-blur-theme dark:border-theme-4">
                    <div ref={refSearchOutside} className="w-[720px] tmd:w-[calc(100%-24px)] rounded-[12px] border dark:border-theme-3 dark:bg-theme-2">
                        <div className="w-[100%] h-[56px] tmd:h-[44px] relative flex flex-row justify-center items-center border-b dark:border-theme-3">
                        <svg className="w-[16px] tmd:w-[12px] absolute left-[24px] tmd:left-[16px] top-[50%] translate-y-[-50%] fill-theme-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/></svg>
                            <input onChange={handleSearch} type="text" placeholder="폰트 검색하기..." autoFocus className="w-[calc(100%-108px)] tmd:w-[calc(100%-84px)] h-[100%] text-[14px] tmd:text-[12px] leading-none dark:text-theme-8 bg-transparent"/>
                            <button onClick={handleCloseBtn} className="w-[36px] h-[24px] rounded-[6px] absolute right-[16px] tmd:right-[12px] top-[50%] translate-y-[-50%] text-[10px] leading-none dark:text-theme-8 dark:bg-theme-3/80 hover:dark:bg-theme-4/60 tlg:hover:dark:bg-theme-3/80 hover:drop-shadow-default hover:dark:drop-shadow-dark tlg:hover:drop-shadow-none">ESC</button>
                        </div>
                            ...
                            {isSuccess && !isLoading && !isRefetching
                                ? ( data.fonts.length !== 0
                                    ? <div className="w-[100%] px-[24px] tmd:px-[16px]">
                                        <div className="text-[18px] tmd:text-[14px] dark:text-theme-8 leading-none mt-[28px] tmd:mt-[24px] mb-[16px] tmd:mb-[12px] font-bold">검색 결과</div>
                                        {data && data.fonts.map((font: {
                                            ...
                                            return (
                                                <a onMouseOver={() => handleLinkMouseOver(idx)} id={activeEl === idx ? "active" : ""} ref={activeEl === idx ? activeRef : null} href={`/DetailPage/${font.code}`} key={font.code} className="search-link w-[100%] h-[60px] tmd:h-[48px] relative px-[16px] mt-[8px] flex flex-row justify-start items-center rounded-[8px] dark:bg-theme-3/80 cursor-pointer">
                                                    <div className="w-[24px] tmd:w-[20px] h-[24px] tmd:h-[20px] border rounded-[6px] tmd:rounded-[4px] flex flex-row justify-center items-center mr-[12px] dark:bg-theme-4 dark:border-theme-4 when-active-1">
                                                    ...
                                                    <div className="text-[14px] tmd:text-[12px] dark:text-theme-8 font-medium leading-none when-active-3">{font.name}</div>
                                                    <div className="text-[14px] dark:text-theme-6 font-normal leading-none tmd:hidden ml-[10px] when-active-4">{font.font_family}</div>
                                                    <div className="text-[12px] dark:text-theme-6 leading-none tmd:hidden ml-[10px] when-active-5">{font.source}</div>
                                    ...
                                    : ( data.fonts.length === 0 && keyword !== ""
                                        ? <div className="w-[100%] h-[100%] absolute left-0 top-0 flex flex-row justify-center items-center text-[16px] tmd:text-[12px] leading-none dark:text-theme-7">&quot;<span className="dark:text-theme-9 font-medium">{keyword}</span>&quot; 에 대한 검색 결과가 없습니다.</div>
                                        : <div className="w-[100%] h-[100%] absolute left-0 top-0 flex flex-row justify-center items-center text-[16px] tmd:text-[12px] leading-none dark:text-theme-7"><span className="dark:text-theme-9 font-medium mr-[8px]">영문/한글 폰트 이름,</span> 또는 <span className="dark:text-theme-9 font-medium ml-[8px]">회사 이름</span>을 검색해 주세요.</div>
                                    )
                                ) : <></>
                            }
                        </div>
                        <div className="w-[100%] h-[48px] flex flex-row justify-end items-center px-[24px] tmd:px-[16px] border-t text-[12px] tmd:text-[10px] dark:text-theme-5 leading-none dark:border-theme-3">© 2023. taedonn, all rights reserved.</div>
                    </div>
                </div> : <></>
            }
        </>
