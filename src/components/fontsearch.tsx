        ...
        <>
            {
                ...
                ? <div className="w-full h-[100vh] fixed left-0 top-0 z-40 flex flex-col justify-start items-center pt-[12vh] tlg:pt-[10vh] tmd:pt-[60px] backdrop-blur bg-blur-theme dark:border-theme-4">
                    <div ref={refSearchOutside} className="w-[720px] tmd:w-[calc(100%-24px)] rounded-[12px] border border-theme-7 dark:border-theme-3 bg-theme-9 dark:bg-theme-2 animate-zoom-in">
                        <div className="w-full h-[56px] tmd:h-[44px] relative flex flex-row justify-center items-center border-b border-theme-7 dark:border-theme-3">
                        <svg className="w-[16px] tmd:w-[12px] absolute left-[24px] tmd:left-[16px] top-[50%] translate-y-[-50%] fill-theme-4 dark:fill-theme-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/></svg>
                            <input onChange={handleSearch} type="text" placeholder="폰트 검색하기..." autoFocus className="w-[calc(100%-108px)] tmd:w-[calc(100%-84px)] h-full text-[14px] tmd:text-[12px] leading-none text-theme-4 dark:text-theme-9 placeholder-theme-5 dark:placeholder-theme-8 bg-transparent"/>
                        ...
                        <div ref={parentRef} className="search-list w-full min-h-[150px] tmd:min-h-[120px] max-h-[500px] relative overflow-auto">
                            {/* 로딩 바 */}
                            {isLoading ? <div className="w-full h-full absolute left-0 top-0 flex flex-row justify-center items-center"><span className="loader w-[40px] tlg:w-[36px] h-[40px] tlg:h-[36px]"></span></div> : <></>}
                            {isRefetching ? <div className="w-full h-full absolute left-0 top-0 flex flex-row justify-center items-center"><span className="loader w-[40px] tlg:w-[36px] h-[40px] tlg:h-[36px]"></span></div> : <></>}

                            {/* 검색 결과 */}
                            {isSuccess && !isLoading && !isRefetching
                                ? ( data.fonts.length !== 0
                                    ? <div className="w-full px-[24px] tmd:px-[16px]">
                                        <div className="text-[18px] tmd:text-[14px] text-theme-3 dark:text-theme-8 leading-none mt-[28px] tmd:mt-[24px] mb-[16px] tmd:mb-[12px] font-bold">검색 결과</div>
                                        {data && data.fonts.map((font: {
                                            ...
                                            return (
                                                <Link aria-label="search-result-link" onMouseOver={() => handleLinkMouseOver(idx)} id={activeEl === idx ? "active" : ""} ref={activeEl === idx ? activeRef : null} href={`/post/${font.font_family.replaceAll(" ", "+")}`} key={font.code} className="search-link w-full h-[60px] tmd:h-[48px] relative px-[16px] mt-[8px] flex flex-row justify-start items-center rounded-[8px] bg-theme-8 dark:bg-theme-3 cursor-pointer">
                                    ...
                                    : ( data.fonts.length === 0 && keyword !== ""
                                        ? <div className="w-full h-full absolute left-0 top-0 flex flex-row justify-center items-center text-[16px] tmd:text-[12px] leading-none text-theme-5 dark:text-theme-7">&quot;<span className="text-theme-3 dark:text-theme-9 font-medium">{keyword}</span>&quot; 에 대한 검색 결과가 없습니다.</div>
                                        : <div className="w-full h-full absolute left-0 top-0 flex flex-row justify-center items-center text-[16px] tmd:text-[12px] leading-none text-theme-5 dark:text-theme-7"><span className="text-theme-3 dark:text-theme-9 font-medium mr-[8px]">영문/한글 폰트 이름,</span> 또는 <span className="text-theme-3 dark:text-theme-9 font-medium ml-[8px]">회사 이름</span>을 검색해 주세요.</div>
                                    )
                                ) : <></>
                            }
                        </div>
                        <div className="w-full h-[48px] flex flex-row justify-end items-center px-[24px] tmd:px-[16px] border-t text-[12px] tmd:text-[10px]  text-theme-5 dark:text-theme-7 leading-none border-theme-7 dark:border-theme-3">© 2023. taedonn, all rights reserved.</div>
                    </div>
                </div> : <></>
            }
        </>
